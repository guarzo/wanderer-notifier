defmodule WandererNotifier.Application.Services.ApplicationService.MetricsTracker do
  ...
  def increment_metric(state, type) do
    new_state = State.update_metrics(state, fn metrics ->
      case type do
        :kill_processed ->
          processing = Map.update(metrics.processing, :kills_processed, 1, &(&1 + 1))
          %{metrics | processing: processing}
        
        :kill_notified ->
          processing = Map.update(metrics.processing, :kills_notified, 1, &(&1 + 1))
          %{metrics | processing: processing}
        
        type when type in @killmail_metrics ->
