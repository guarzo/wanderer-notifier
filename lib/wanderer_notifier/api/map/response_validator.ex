defmodule WandererNotifier.Api.Map.ResponseValidator do
    ...
    case response do
      %{"data" => data} when is_list(data) ->
        ...
        if Enum.all?(data, &valid_system?/1) do
          ...
          Logger.warning("[ResponseValidator] #{length(invalid_systems)} systems have invalid format")
          # Log one example of an invalid system for debugging
          if length(invalid_systems) > 0 do
            Logger.debug("[ResponseValidator] Example invalid system: #{inspect(List.first(invalid_systems))}")
          end
      ...
      %{"systems" => systems} when is_list(systems) ->
        ...
        Logger.warning("[ResponseValidator] Legacy systems response format detected with 'systems' key")
        # Check that each system has the required fields
    ...
    case response do
      %{"data" => data} when is_list(data) ->
        ...
        if Enum.all?(data, &valid_character?/1) do
          ...
          Logger.warning("[ResponseValidator] #{length(invalid_characters)} characters have invalid format")
          # Log one example of an invalid character for debugging
          if length(invalid_characters) > 0 do
            Logger.debug("[ResponseValidator] Example invalid character: #{inspect(List.first(invalid_characters))}")
          end
      ...
      %{"characters" => characters} when is_list(characters) ->
        # Legacy format support
        Logger.warning("[ResponseValidator] Legacy characters response format detected with 'characters' key")
        # Check that each character has the required fields
    ...
    case response do
      %{"data" => data} when is_list(data) ->
        ...
        if Enum.all?(data, &valid_activity?/1) do
          ...
          Logger.warning("[ResponseValidator] #{length(invalid_entries)} activity entries have invalid format")
          # Log one example of an invalid entry for debugging
          if length(invalid_entries) > 0 do
            Logger.debug("[ResponseValidator] Example invalid activity entry: #{inspect(List.first(invalid_entries))}")
          end
      ...
      %{"activity" => activity} when is_list(activity) ->
        # Legacy format support
        Logger.warning("[ResponseValidator] Legacy activity response format detected with 'activity' key")
        # Check that each activity entry has the required fields
      ...
      _ ->
        Logger.error("[ResponseValidator] Expected 'data' or 'activity' array in activity response")
  ...
  def validate_system_static_info_response(response) do
    case response do
      %{"data" => data} when is_map(data) ->
        # Check that the static info has the required fields
        if valid_static_info?(data) do
          {:ok, data}
        else
          Logger.warning("[ResponseValidator] System static info has invalid format")
          {:error, "System static info has invalid format"}
        end
        
      %{"static_info" => static_info} when is_map(static_info) ->
        # Legacy format support
        Logger.warning("[ResponseValidator] Legacy static info response format detected with 'static_info' key")
      ...
      _ ->
        Logger.error("[ResponseValidator] Expected 'data' or 'static_info' object in static info response")
        {:error, "Expected 'data' or 'static_info' object in response"}
    end
  end
  ...
  defp valid_system?(system) do
    ...
    (is_integer(Map.get(system, "solar_system_id")) or is_binary(Map.get(system, "solar_system_id", ""))) and
  ...
  defp valid_character?(character) do
    ...
    (is_binary(get_in(character, ["character", "eve_id"], "")) or is_integer(get_in(character, ["character", "eve_id"], 0)))
  end
  ...
  defp valid_activity?(activity) do
    ...
    (is_binary(get_in(activity, ["character", "eve_id"], "")) or is_integer(get_in(activity, ["character", "eve_id"], 0))) and
  ...
  defp valid_static_info?(info) do
    is_map(info) and
    is_list(Map.get(info, "statics", [])) and
    (is_binary(Map.get(info, "security", "")) or is_number(Map.get(info, "security", 0.0))) and
    (is_integer(Map.get(info, "solar_system_id", 0)) or is_binary(Map.get(info, "solar_system_id", "")))
  end
