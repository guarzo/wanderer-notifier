defmodule WandererNotifier.Api.Http.ErrorHandler do
  ...
  def classify_error(error) do
    case error do
      ...
      {:econnrefused, _} -> :transient
      {:closed, _} -> :transient
      {:timeout, _} -> :transient
      :econnrefused -> :transient
      :enetunreach -> :transient
      :system_limit -> :transient
      ...
      :json_error -> :permanent
      :unsupported_media_type -> :permanent
      :conflict -> :permanent
  ...
  """
  @spec annotate_error(http_error(), atom(), any()) :: {:domain_error, atom(), {http_error(), any()}}
  ...
  defp handle_esi_error(status_or_error, body_or_reason) do
    error = if is_integer(status_or_error), do: status_to_error(status_or_error), else: status_or_error
