defmodule WandererNotifier.Cache.Monitor do
  ...
  defp run_cache_health_check do
    Logger.info("[CacheMonitor] Running cache health check")
    ...
    e ->
      Logger.error("[CacheMonitor] Error during cache health check: #{Exception.message(e)}")
      Logger.debug("[CacheMonitor] #{Exception.format_stacktrace()}")
      %{error: e}
  end
  ...
  defp log_health_check_results(results) do
    if results.character_count_inconsistency || length(results.missing_characters) > 0 || length(results.different_characters) > 0 do
      ...
      Logger.info("[CacheMonitor] Cache health check completed - no inconsistencies found")
    end
  end
  ...
  defp check_tracked_characters(results) do
    ...
    case TrackedCharacter.list_all() do
      ...
      {:error, reason} ->
        Logger.error("[CacheMonitor] Error retrieving characters from database: #{inspect(reason)}")
        results
    end
  end
  ...
  defp compare_characters(cached_characters, db_characters) do
    ...
    if length(missing_in_cache) > 0 do
      Logger.warning("[CacheMonitor] Found #{length(missing_in_cache)} characters in database but missing from cache")
    end

    if length(different_data) > 0 do
      Logger.warning("[CacheMonitor] Found #{length(different_data)} characters with data mismatches between cache and database")
    end
  ...
  defp resync_characters_cache(db_characters) do
    Logger.info("[CacheMonitor] Resyncing characters cache from database (#{length(db_characters)} characters)")
    ...
    Logger.info("[CacheMonitor] Characters cache successfully resynced from database")
  end
