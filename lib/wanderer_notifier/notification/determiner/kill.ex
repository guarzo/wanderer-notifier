defmodule WandererNotifier.Notification.Determiner.Kill do
  @moduledoc """
  Determines whether kill notifications should be sent.
  Handles all kill-related notification decision logic.
  """

  @behaviour WandererNotifier.Notification.Behaviour
  ...
  @impl true
  def determine(killmail) do
    case should_notify?(killmail) do
      {:ok, %{should_notify: true}} -> {:ok, build_notification_data(killmail)}
      {:ok, %{should_notify: false}} -> {:ok, nil}
  ...
  def format(notification_data) do
    # Delegate to the appropriate formatter
    WandererNotifier.Notification.Formatter.Kill.format(notification_data)
  end
  ...
  defp check_attackers_tracked(kill_data, all_character_ids) do
    ...
    Enum.any?(attackers, fn attacker ->
      ...
      if attacker_id_str do
        Enum.member?(all_character_ids, attacker_id_str) ||
          check_direct_victim_tracking(attacker_id_str)
      else
        false
      end
    end)
  ...
  # Build notification data from killmail
  defp build_notification_data(killmail) do
    %{
      type: :kill,
      killmail: killmail,
      system_id: get_kill_system_id(killmail),
