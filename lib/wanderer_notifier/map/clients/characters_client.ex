defmodule WandererNotifier.Map.Clients.CharactersClient do
  ...
  """
  @spec update_tracked_characters([character()]) :: update_result()
  def update_tracked_characters(cached \\ []) do
    url     = characters_url()
    headers = auth_header()
    ...
    result =
      with {:ok, %{status_code: 200, body: body}} <- HttpClient.get(url, headers),
        ...
        process_and_cache(chars, cached)
      end
  ...
  defp process_and_cache(chars, cached) do
  ...
  defp safe_cache(chars) do
    ...
    Logger.api_info("CharactersClient cached #{length(chars)} characters")
  rescue
    e ->
