defmodule WandererNotifier.Map.SystemsClient do
  ...
  """
  @spec update_systems(list() | nil) :: {:ok, list()} | {:error, term()}
  def update_systems(cached_systems \\ nil) do
    AppLogger.api_info("📊 Starting systems update")

    # Fetch systems from API
    case request_systems() do
      {:ok, systems_data} ->
        # Extract systems from the response
        systems_data = extract_systems_data(systems_data)

        # Filter systems based on configuration
        tracked_systems = filter_systems_for_tracking(systems_data)

        # Enrich systems with static data
        enriched_systems = enrich_tracked_systems(tracked_systems)

        # Update systems in cache
        updated_systems = update_systems_cache(enriched_systems)

        # Verify systems are cached properly
        verify_systems_cached(updated_systems)

        # Handle notifications for new systems if needed
        if is_list(cached_systems) do
          notify_new_systems(updated_systems, cached_systems)
        else
          AppLogger.api_debug(
            "[SystemsClient] No cached systems provided, skipping notification check"
          )
        end
      ...
      {:error, reason} ->
        AppLogger.api_error("[SystemsClient] Failed to update systems", error: inspect(reason))
        {:error, reason}
    end
  end

  # Make API request to fetch systems
  defp request_systems do
    # Log the request start
    AppLogger.api_debug("[SystemsClient] Requesting systems from API")

    # Make API request with proper error handling
    result = HttpClient.get("map_systems")
    ...
    case result do
      {:ok, %{status_code: 200, body: body}} ->
        try do
          ...
          AppLogger.api_debug("[SystemsClient] Successfully fetched systems data",
            data_type: typeof(decoded),
            size: byte_size(body)
          )
          ...
          e ->
            AppLogger.api_error("[SystemsClient] Failed to decode systems response",
      ...
      {:ok, %{status_code: status_code}} ->
        AppLogger.api_error("[SystemsClient] Failed to fetch systems with status",
          status: status_code
        )
      ...
      {:error, reason} ->
        AppLogger.api_error("[SystemsClient] Network error fetching systems",
  ...
  defp enrich_tracked_systems(tracked_systems) do
    ...
    system_map =
      Enum.reduce(tracked_systems, %{}, fn sys, acc ->
        system_id = extract_system_id(sys)
        if system_id, do: Map.put(acc, system_id, MapSystem.new(sys)), else: acc
      end)
  ...
  defp process_systems_for_enrichment(tracked_systems, system_map) do
    Enum.reduce(tracked_systems, system_map, fn system, acc ->
      ...
      if system_id && Map.has_key?(acc, system_id) do
        process_system(Map.get(acc, system_id), acc)
      else
        acc
      end
    end)
  ...
  defp process_system(system, acc) do
    ...
    case Task.yield(task, 2_000) do
      ...
      _ ->
        ...
        updated_system =
          if MapSystem.is_wormhole?(system) do
  ...
  # Find systems that weren't in the cached systems but are in the current systems
  ...
  # Handle notification results and log failures
  ...
  @doc """
  Sends a notification for a system.

  ## Parameters
    - validated_system: A validated MapSystem struct

  ## Returns
    - :ok on success
    - {:error, reason} on failure
  """
  @spec send_system_notification(MapSystem.t()) :: :ok | {:error, term()}
  def send_system_notification(%MapSystem{} = validated_system) do
    # Convert to Data.MapSystem for structured formatter compatibility
    converted_system = convert_to_data_map_system(validated_system)

    # Format the system as a notification
    generic_notification = StructuredFormatter.format_system_notification(converted_system)

    # Send the notification through the factory
    Factory.send_system_notification(generic_notification)
  end
  ...
  defp convert_to_data_map_system(system) do
    %WandererNotifier.Data.MapSystem{
      ...
      system_type: get_system_type_from_security(system),
      solar_system_id: system.solar_system_id || system.id,
      statics: system.statics || []
    }
  end

  # Determine system type from security information
  defp get_system_type_from_security(system) do
    cond do
      is_binary(system.security_status) && String.starts_with?(system.security_status, "C") ->
        "wormhole"

      is_nil(system.security_status) ->
        "unknown"
