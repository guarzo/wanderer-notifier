defmodule WandererNotifier.Logger.StructuredLogger do
  ...
  defp extract_from_value(:killmail, %{} = killmail) do
    ...
    base = if kill_id = safe_get_in(killmail, ["killmail_id"]) || safe_get_in(killmail, [:killmail_id]),
      do: [{MetadataKeys.kill_id(), kill_id} | base],
      else: base
      
    base = if system_id = safe_get_in(killmail, ["solar_system_id"]) || safe_get_in(killmail, [:solar_system_id]),
      do: [{MetadataKeys.system_id(), system_id} | base],
      else: base
      
    base = if character_id = safe_get_in(killmail, ["victim", "character_id"]) || safe_get_in(killmail, [:victim, :character_id]),
  ...
  defp extract_from_value(:character, %{} = character) do
    ...
    base = if character_id = safe_get_in(character, ["character_id"]) || safe_get_in(character, [:character_id]),
      do: [{MetadataKeys.character_id(), character_id} | base],
      else: base
      
    base = if corp_id = safe_get_in(character, ["corporation_id"]) || safe_get_in(character, [:corporation_id]),
      do: [{MetadataKeys.corporation_id(), corp_id} | base],
      else: base
      
    base = if alliance_id = safe_get_in(character, ["alliance_id"]) || safe_get_in(character, [:alliance_id]),
  ...
  defp extract_from_value(:system, %{} = system) do
    ...
    base = if system_id = safe_get_in(system, ["system_id"]) || safe_get_in(system, [:system_id]),
      do: [{MetadataKeys.system_id(), system_id} | base],
      else: base
      
    base = if system_name = safe_get_in(system, ["name"]) || safe_get_in(system, [:name]),
  ...
  defp extract_from_value(:response, %{} = response) do
    ...
    base = if status = safe_get_in(response, [:status]) || safe_get_in(response, [:status_code]),
      do: [{MetadataKeys.status_code(), status} | base],
      else: base
      
    base = if url = safe_get_in(response, [:url]) || safe_get_in(response, [:request_url]),
