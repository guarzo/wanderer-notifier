defmodule WandererNotifier.ChartService do
  ...
  def send_chart_to_discord(image_binary, title, description, channel_id)
    ...
    if url?(image_binary) do
      ...
      embed = %{
        :title => title,
        :description => description || "Chart generated by WandererNotifier",
        # Discord blue
        :color => 3_447_003,
        :image => %{
          :url => chart_url
      ...
      case NeoClient.send_embed(embed, channel_id) do
        ...
        {:error, %{status_code: code, response: response}} ->
          AppLogger.api_error("Discord API error", status_code: code, response: inspect(response))
          {:error, "Discord API error: #{code}"}

        {:error, reason} ->
          AppLogger.api_error("Failed to send embed", error: inspect(reason))
          {:error, "Failed to send embed to Discord"}
      end
    else
      # It's binary image data, send it directly as a file
      AppLogger.api_debug("Binary image data detected - sending as file", chart_title: title)
      ...
      # Send the file to Discord with the embed
      case NeoClient.send_file(filename, image_binary, title, description, channel_id, embed) do
        :ok ->
          AppLogger.api_info("Successfully sent chart to Discord", title: title)
          {:ok, %{title: title}}
  ...
  def send_chart_to_discord(config, title, description, channel_id) do
    # Try to generate a chart using the Node.js service first
    case generate_chart_image(config) do
      {:ok, image_binary} ->
        # Create rich embed with the chart as an attachment
        embed = %{
          :title => title,
          :description => description || "Chart generated by WandererNotifier",
          # Discord blue
        ...
        # Send the file to Discord with the embed
        case NeoClient.send_file(filename, image_binary, title, description, channel_id) do
          :ok -> {:ok, %{title: title}}
          {:error, reason} -> {:error, reason}
        end

      {:error, _} ->
        # Fall back to QuickChart URL generation
      ...
      {:ok, json_config} ->
        ...
        case Base.encode64(json_config) do
          encoded_config when is_binary(encoded_config) ->
            ...
            case url_result do
              {:ok, url} ->
                AppLogger.api_debug("Successfully built chart URL",
              ...
              error ->
                AppLogger.api_error("Failed to build chart URL", error: inspect(error))
                error
            end
