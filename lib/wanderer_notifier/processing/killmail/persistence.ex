defmodule WandererNotifier.Processing.Killmail.Persistence do
  ...
  defp check_killmail_exists(killmail_id) do
  ...
  defp create_character_involvements(killmail, killmail_record_id) do
    # Extract all character involvements
    involvements = extract_character_involvements(killmail, killmail_record_id)

    # Insert all involvements in a batch
    case insert_character_involvements(involvements) do
      {:ok, inserted_count} ->
        AppLogger.kill_debug(
          "Created #{inserted_count} character involvements for killmail ##{killmail.killmail_id}"
        )
  ...
  # Extract character involvements from the killmail
  defp extract_character_involvements(killmail, killmail_record_id) do
  ...
  defp track_processing(killmail, character_id) do
