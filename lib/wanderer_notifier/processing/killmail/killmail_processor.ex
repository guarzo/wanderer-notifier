defmodule WandererNotifier.Processing.Killmail.KillmailProcessor do
  ...
  alias WandererNotifier.Processing.Killmail.NotificationDeterminer
  ...
  1. Normalizes the input data to KillmailData format
  ...
  # Normalizes any killmail format to the standard KillmailData struct
  defp normalize_killmail(killmail) do
    ...
    cond do
      # Already a KillmailData struct, return as is
      is_struct(killmail, KillmailData) ->
        {:ok, killmail}

      # Has both killmail_id and hash, fetch from ESI and create KillmailData
  ...
  defp extract_killmail_id(killmail) do
    ...
    cond do
      is_struct(killmail, KillmailData) -> killmail.killmail_id
  ...
  defp extract_hash(killmail) do
    # Simple extraction initially - will be enhanced
    cond do
      is_struct(killmail, KillmailData) && is_binary(killmail.zkb_hash) ->
        killmail.zkb_hash

      is_struct(killmail, KillmailData) && is_map(killmail.raw_zkb_data) ->
  ...
  # Fetch ESI data and create a KillmailData struct
  defp fetch_esi_data_and_create_killmail(kill_id, hash, zkb_data) do
    ...
    case ESIService.get_killmail(kill_id, hash) do
      {:ok, esi_data} ->
        # Create a KillmailData struct from the zkb and ESI data
        KillmailData.from_zkb_and_esi(zkb_data, esi_data)
  ...
  """
  def configure do
    AppLogger.info("Configuring KillmailProcessor module")
