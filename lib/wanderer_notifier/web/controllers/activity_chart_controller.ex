defmodule WandererNotifier.Web.Controllers.ActivityChartController do
  ...
  get "/generate/:chart_type" do
    ...
    if chart_type == "invalid" do
      ...
      chart_result = case chart_type do
        "activity_summary" -> ActivityChartAdapter.generate_activity_summary_chart(activity_data)
  ...
  get "/send-all" do
    ...
    Logger.info("Sending all charts with #{if map_slug == nil || map_slug == "", do: "mock", else: "real"} data")

    # Get character activity data
    activity_data = case MapClient.get_character_activity(map_slug) do
      {:ok, data} -> data
      _ -> nil
    end
    ...
    # Report the results
    Logger.info("Completed sending all activity charts to Discord. Success: #{success_count}/#{length(results)}")

    # Format the results in a way that can be encoded to JSON
    formatted_results = Enum.map(results, fn {chart_type, result} ->
      case result do
        {:ok, url, title} -> %{chart_type: chart_type, status: "success", url: url, title: title}
        {:error, message} -> %{chart_type: chart_type, status: "error", message: message}
        _ -> %{chart_type: chart_type, status: "error", message: "Unknown error"}
      end
    end)
  ...
  get "/character-activity" do
    ...
    response = case MapClient.get_character_activity(Config.map_name()) do
      {:ok, data} ->
        ...
        characters = cond do
          ...
          is_map(data) && Map.has_key?(data, "data") && is_list(data["data"]) ->
            Logger.info("Found data structure: map with 'data' key containing a list of #{length(data["data"])} records")
            data["data"]

          # If data is a map with a "data" key that contains a map with a "characters" key
          is_map(data) && Map.has_key?(data, "data") && is_map(data["data"]) && Map.has_key?(data["data"], "characters") ->
            char_data = data["data"]["characters"]
            Logger.info("Found data structure: nested map with characters key containing #{length(char_data)} records")
            char_data
      ...
      _ ->
        ...
        conn
        |> put_resp_content_type("application/json")
        |> send_resp(500, Jason.encode!(%{status: "error", message: "Failed to retrieve activity data"}))
    end
  ...
  defp send_chart_to_discord(chart_type, activity_data) do
    ...
    Logger.info("Activity data type: #{if is_nil(activity_data), do: "nil", else: if(is_map(activity_data), do: "map with keys: #{inspect(Map.keys(activity_data))}", else: if(is_list(activity_data), do: "list with #{length(activity_data)} items", else: inspect(activity_data, limit: 50)))}")

    # If we don't have activity data, try to fetch it
    activity_data = if is_nil(activity_data) do
    ...
    case chart_type do
      :activity_summary ->
        ActivityChartAdapter.send_chart_to_discord(activity_data, "Character Activity Summary")
      :activity_timeline ->
        ActivityChartAdapter.send_activity_timeline_chart(activity_data)
      :activity_distribution ->
        ActivityChartAdapter.send_activity_distribution_chart(activity_data)
      _ ->
