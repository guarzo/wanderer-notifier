defmodule ChainKills.Discord.Notifier do
  ...
  def send_new_tracked_character_notification(character) when is_map(character) do
    ...
    description = "New tracked character **#{name}** (Corp: #{corp}) has been retrieved from the API."
  ...
  defp resolve_system_name(solar_system_id) do
    ...
    case Enum.find(tracked, fn sys ->
      ...
      system ->
        Map.get(system, "system_name") || Map.get(system, :alias) || "Solar System #{solar_system_id}"
    end
  end
