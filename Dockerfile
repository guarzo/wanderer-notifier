...
RUN echo '#!/bin/sh' > /app/generate_config.sh && \
    ...
    echo 'TOKEN_BIN="<<\\\"$DISCORD_BOT_TOKEN\\\">>"' >> /app/generate_config.sh && \
    echo 'CONFIG="[{kernel, [{distribution_mode, none}, {start_distribution, false}]}, {nostrum, [{token, $TOKEN_BIN}]}]."' >> /app/generate_config.sh && \
    echo 'echo "$CONFIG" > "$RELEASE_DIR/sys.config"' >> /app/generate_config.sh && \
    echo 'echo "$CONFIG" > "$RELEASE_DIR/sys/app/releases/sys.config"' >> /app/generate_config.sh && \
    echo 'echo "$CONFIG" > "/app/releases/sys.config"' >> /app/generate_config.sh && \
    echo 'find /app -name sys.config | xargs ls -la' >> /app/generate_config.sh && \
    echo 'echo "Generated sys.config files"' >> /app/generate_config.sh && \
