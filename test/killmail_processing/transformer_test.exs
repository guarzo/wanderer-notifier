defmodule WandererNotifier.KillmailProcessing.TransformerTest do
  ...
  describe "to_killmail_data/1" do
    test "returns same struct if already a KillmailData" do
    ...
    test "converts from KillmailResource" do
      ...
      result = Transformer.to_killmail_data(resource)

      assert %KillmailData{} = result
    ...
    test "converts from raw map with atom keys" do
      ...
      result = Transformer.to_killmail_data(raw_data)
    ...
    test "handles nil and invalid inputs" do
      assert Transformer.to_killmail_data(nil) == nil
      assert Transformer.to_killmail_data(123) == nil
      assert Transformer.to_killmail_data("not a map") == nil
    end
  end
